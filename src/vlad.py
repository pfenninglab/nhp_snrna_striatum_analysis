...
#Copied directly from https://github.com/moonfolk/VLAD
import numpy as np
...
def vlad_centers_given_2(X, K, center_indices,alpha=None,  n_jobs=1, deconv=True, lda=False, pois=False, alpha_m_path=None,
         Nm=None):
    cent = X.mean(axis=0)
    ...
    v_back = s.reshape(-1, 1) * v[:K]
    X_svd = u[:,:K]
    u_centers= X_svd[center_indices,:]
    ...
    if alpha is None:
        ...
        if alpha_m_path is None:
            print(
                'Make grid for learning alpha - this may take some time. Needs to be done once for a fixed K (independent of data)')
            from joblib import Parallel, delayed
            alpha_m = Parallel(n_jobs=n_jobs)(delayed(simul_m)(K, a) for a in alpha_range)
            alpha_m = [a_m[0] for a_m in alpha_m]
            np.save('alpha_m_%d' % K, np.array(alpha_m))
